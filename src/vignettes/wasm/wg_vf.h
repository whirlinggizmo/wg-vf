 ...
 *
 * Pointers/offsets are uintptr_t: a 32-bit linear-memory offset on wasm32, a
 * native pointer on a 64-bit host. Everything else (ids, lengths, step values,
 * return codes) is uint32_t on every target.
 */
